...
def evaluate_with_rules(
    ...
    """用规则指标对单篇报告评分。"""
    return bench.evaluate_report(report, qid, report_metadata=report_metadata)
    ...
    for q in questions:
        ...
        try:
            _, report_obj = asyncio.run(
                run_research(query, cfg, modules, return_report=True)
            )
            elapsed = time.time() - start
            eval_result = evaluate_with_rules(
                report_obj,
                qid,
                bench,
            ...
            details.append({
                ...
                "evidence_metrics": eval_result.get("evidence_metrics", {}),
        ...
        "systems": [
            {
                ...
                "average_metrics": r.get("average_metrics", {}),
                "details": r["details"],
            }
