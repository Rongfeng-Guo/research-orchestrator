...
async def run_agent(query: str, config: dict) -> dict:
    ...
    report_md, report_obj = await run_research(query, config, modules, return_report=True)
    metadata = getattr(report_obj, "metadata", {}) or {}

    return {
        "query": query,
        "content": getattr(report_obj, "content", report_md),
        "length": len(getattr(report_obj, "content", report_md)),
        "source_count": len(getattr(report_obj, "sources", []) or []),
        "confidence": getattr(report_obj, "confidence", 0.0),
        "report_metadata": metadata,
        "evidence_graph_quality": RuleBasedMetrics.evidence_graph_quality(metadata),
    }
