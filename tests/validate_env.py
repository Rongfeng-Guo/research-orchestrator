...
mimo_configured = any(
    os.getenv(k) for k in ("MIMO_API_KEY", "MIMO_BASE_URL", "MIMO_MODEL")
)
print(check("MIMO_API_KEY", optional=True))
print(check("MIMO_BASE_URL", expected_prefix="https://", optional=not mimo_configured))
print(check("MIMO_MODEL", optional=not mimo_configured))
...
if not mimo_configured:
    print("  ✅ MIMO 未配置；运行时缺失的角色后端会自动回退到默认后端")
elif "openrouter" in mimo_url and not mimo_model.startswith("xiaomi/"):
    print("  ⚠️  MIMO 配置不匹配：")
...
if search_backend == "serpapi" and not os.getenv("SERPAPI_KEY"):
    print("  ⚠️  SEARCH_BACKEND=serpapi，但 SERPAPI_KEY 未设置；运行时会自动回退到 mock 搜索")
elif search_backend == "bing" and not os.getenv("BING_SEARCH_KEY"):
    print("  ⚠️  SEARCH_BACKEND=bing，但 BING_SEARCH_KEY 未设置；运行时会自动回退到 mock 搜索")
else:
    print("  ✅ 搜索后端配置正确")
