...
def test_build_iteration_plan_uses_stable_paths_and_commands() -> None:
    ...
    assert plan["paths"]["cache_file"] == _path("data", "search_cache", "real", "real_test.jsonl")
    assert plan["paths"]["resurfaced_file"] == _path(
        "data",
        "search_cache",
        "real_resurfaced",
        "real_test_resurfaced.jsonl",
    )
    assert plan["paths"]["analysis_json"] == _path(
        "outputs",
        "analysis_real",
        "search_cache_analysis_real_test_resurfaced.json",
    )
    assert plan["paths"]["train_summary_file"] == _path("outputs", "train_real", "real_test_train_summary.json")
    assert plan["paths"]["model_file"] == _path("artifacts", "search_policy_real_test.json")
    assert plan["paths"]["audit_dir"] == _path("outputs", "audit_real")
...
def test_build_iteration_plan_supports_train_and_heldout_splits() -> None:
    ...
    assert plan["paths"]["cache_file"] == _path("data", "search_cache", "real", "real_test_train.jsonl")
    assert plan["paths"]["heldout_cache_file"] == _path("data", "search_cache", "real", "real_test_heldout.jsonl")
    assert plan["paths"]["resurfaced_file"] == _path(
        "data",
        "search_cache",
        "real_resurfaced",
        "real_test_train_resurfaced.jsonl",
    )
    assert plan["paths"]["heldout_resurfaced_file"] == _path(
        "data",
        "search_cache",
        "real_resurfaced",
        "real_test_heldout_resurfaced.jsonl",
    )
    assert plan["paths"]["analysis_jsons"][0].startswith(
        _path("outputs", "analysis_real", "search_cache_analysis_real_test_train")
    )
    assert plan["paths"]["analysis_jsons"][1].startswith(
        _path("outputs", "analysis_real", "search_cache_analysis_real_test_heldo")
    )
    ...
    assert plan["commands"]["train_policy"][plan["commands"]["train_policy"].index("--inputs") + 1] == (
        _path("data", "search_cache", "real_resurfaced", "real_test_train_resurfaced.jsonl")
    )
