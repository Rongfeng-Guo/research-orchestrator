...
4. Generate {min_subtasks} to {max_subtasks} sub_tasks
...
11. Do NOT copy full raw URLs into search_hints. Use short keywords or source names instead.
...
class Planner:
    ...
    def __init__(
        ...
        self.min_subtasks = max(1, int(min_subtasks))
        self.max_subtasks = max(self.min_subtasks, int(max_subtasks))
    ...
    def generate_plan(self, query: str, memory_context: str = "") -> DAG:
        ...
        try:
            ...
            return self._build_fallback_plan(query, memory_context)
    ...
    def replan(
        ...
        try:
            ...
            return self._build_fallback_plan(query, reason=reason)
    ...
    def _build_prompt(self, query: str, memory: str) -> str:
        ...
        if not has_memory:
            extra_hint = (
                ...
                f"Please decompose the task thoroughly, but stay within {self.min_subtasks}-{self.max_subtasks} sub_tasks. "
                "Each sub-task should focus on a distinct angle or data source.\n"
    ...
    def _parse_plan(self, json_str: str) -> DAG:
        ...
        解析策略:
          ...
          4. 验证 DAG 无环
        """
        data = self._load_plan_payload(json_str)
    ...
    def get_task_map_from_dag(self, dag: DAG, raw_json: str) -> dict[str, SubTask]:
        ...
        """
        try:
            data = self._load_plan_payload(raw_json)
        except PlanParseError:
            return {}
    ...
    def _build_fallback_plan(
        ...
        query: str,
        memory_context: str = "",
        reason: str = "planner_parse_failure",
    ) -> DAG:
        task = {
            "task_id": "task_1",
            "task_type": "search",
            ...
            "priority": 1,
            "expected_type": "factual",
            "search_hints": self._derive_search_hints(query),
            "fallback_reason": reason,
            "memory_context_used": bool(memory_context and memory_context.strip()),
        }
        self._last_raw_json = json.dumps({"sub_tasks": [task]}, ensure_ascii=False, indent=2)
        return self._parse_plan(self._last_raw_json)

    @staticmethod
    def _derive_search_hints(query: str, max_hints: int = 6) -> list[str]:
        sanitized = re.sub(r"https?://\S+", " ", query)
        pieces = re.split(r"[\s,;:()\[\]{}]+", sanitized)
        hints: list[str] = []
        seen: set[str] = set()
        stopwords = {
            "analyze",
        ...
        for piece in pieces:
            ...
            if len(hints) >= max_hints:
                break
        return hints or ["official source", "primary evidence"]

    @staticmethod
    def _extract_json_candidate(json_str: str) -> str:
        raw = json_str.strip()
    ...
    def _load_plan_payload(self, json_str: str) -> dict[str, Any]:
        raw = self._extract_json_candidate(json_str)

        try:
            return json.loads(raw)
        except json.JSONDecodeError as first_error:
            cleaned = self._strip_json_line_comments(raw)
            try:
                ...
                try:
                    ...
                    ) from repair_error if repair_error is not None else first_error
