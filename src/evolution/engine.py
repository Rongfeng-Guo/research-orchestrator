...
class SelfEvolutionEngine:
    ...
    def __init__(
        ...
        self.evidence_dataset_builder = EvidenceTransitionDatasetBuilder()
    ...
    async def run_round(self) -> dict[str, Any]:
        ...
        round_stats["avg_transition_count"] = (
            sum(item.get("transition_count", 0) for item in collected_batch) / len(collected_batch)
            if collected_batch
            else 0.0
        )
        round_stats["avg_process_reward"] = (
            sum(
                (sum(item.get("process_reward_trace", [])) / len(item.get("process_reward_trace", [])))
                for item in collected_batch
                if item.get("process_reward_trace")
            )
            / max(len([item for item in collected_batch if item.get("process_reward_trace")]), 1)
        )
        ...
        try:
            dataset_rows = self.evidence_dataset_builder.build_from_collected_batch(collected_batch)
            dataset_path = os.path.join(round_dir, "evidence_transition_dataset.jsonl")
            self.evidence_dataset_builder.export_jsonl(dataset_rows, dataset_path)
            dataset_summary = self.evidence_dataset_builder.summarize(dataset_rows)
            round_stats["evidence_dataset_path"] = dataset_path
            round_stats["evidence_dataset_examples"] = len(dataset_rows)
            round_stats["evidence_dataset_summary"] = dataset_summary
            logger.info(
                f"[Round {self.current_round}] Evidence transition dataset saved to "
                f"{dataset_path} ({len(dataset_rows)} rows)"
            )
        except Exception as e:
            logger.warning(f"Failed to export evidence transition dataset: {e}")
            round_stats["evidence_dataset_path"] = ""
            round_stats["evidence_dataset_examples"] = 0
            round_stats["evidence_dataset_summary"] = {"num_examples": 0}
