...
class SymbolicLearner:
    ...
    def _compress_trajectories(self, trajectories: list[dict[str, Any]]) -> str:
        ...
        for i, traj in enumerate(trajectories[:20]):  # 最多取 20 条
            ...
            transition_trace = traj.get("evidence_transition_trace", [])
            process_rewards = traj.get("process_reward_trace", [])
            parts.append(
            ...
            if isinstance(transition_trace, list) and transition_trace:
                first = transition_trace[0].get("before", {}) if isinstance(transition_trace[0], dict) else {}
                last = transition_trace[-1].get("after", {}) if isinstance(transition_trace[-1], dict) else {}
                parts.append(
                    "过程轨迹: "
                    f"steps={len(transition_trace)}, "
                    f"uncertainty {first.get('uncertainty', 'n/a')} -> {last.get('uncertainty', 'n/a')}, "
                    f"coverage {first.get('coverage_ratio', 'n/a')} -> {last.get('coverage_ratio', 'n/a')}, "
                    f"conflicts {first.get('open_conflicts', 'n/a')} -> {last.get('open_conflicts', 'n/a')}\n"
                )
            if isinstance(process_rewards, list) and process_rewards:
                parts.append(f"过程奖励: {process_rewards[:8]}\n")
        return "\n".join(parts)
