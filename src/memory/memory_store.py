...
from src.evidence.policy import (
...
class SharedMemoryStore:
    ...
    def __init__(
        ...
        session_id: str = "",
        evidence_policy: EvidenceAwareResearchPolicy | None = None,
    ) -> None:
        """
        初始化共享记忆存储。
        ...
        self._evidence_policy = evidence_policy or EvidenceAwareResearchPolicy()
    ...
    def get_evidence_snapshot(
        ...
        top_k: int = 8,
        min_sim: float = 0.55,
        entries_with_sim: list[tuple[MemoryEntry, float]] | None = None,
    ) -> EvidenceSnapshot:
        """Build an evidence-state snapshot for the current query."""
        if entries_with_sim is None:
            entries_with_sim = self.query_by_similarity(query, top_k=top_k, min_sim=min_sim)
        ...
        selected_ids = {node.entry_id for node in nodes}
        open_conflicts: list[EvidenceConflict] = []
        if selected_ids:
            for conflict in self.get_conflicts(status="open"):
                if conflict.entry_id_1 in selected_ids or conflict.entry_id_2 in selected_ids:
                    open_conflicts.append(
                        EvidenceConflict(
                            conflict_id=conflict.conflict_id,
                            entry_id_1=conflict.entry_id_1,
                            entry_id_2=conflict.entry_id_2,
                            claim_1=conflict.claim_1,
                            claim_2=conflict.claim_2,
        ...
        return self._evidence_policy.analyze(
            query=query,
            nodes=nodes,
...
请只输出 A 或 B："""
    ...
    def get_context_for_query(self, query: str, max_tokens: int = 4000) -> str:
        ...
        snapshot = self.get_evidence_snapshot(
            query,
            top_k=10,
            min_sim=0.55,
            entries_with_sim=entries_with_sim,
        )
        ...
        snapshot_block = snapshot.to_markdown()
        if snapshot_block:
            parts.append(snapshot_block + "\n")
            current_chars += len(snapshot_block) + 1
