...
class ResearchBench:
    ...
    def _extract_report_payload(
        ...
        """兼容 str / ResearchReport / dict 三种输入形式。"""
        text = ""
        metadata: dict[str, Any] = {}
        sources: list[dict[str, Any]] = []

        if isinstance(report, dict):
            text = str(report.get("content", report.get("report_content", "")) or "")
            maybe_metadata = report.get("metadata") or report.get("report_metadata") or {}
            if isinstance(maybe_metadata, dict):
                metadata.update(maybe_metadata)
            maybe_sources = report.get("sources", [])
            if isinstance(maybe_sources, list):
                sources = [item for item in maybe_sources if isinstance(item, dict)]
        elif hasattr(report, "content") and not isinstance(report, str):
            text = str(getattr(report, "content", "") or "")
            maybe_metadata = getattr(report, "metadata", {})
            if isinstance(maybe_metadata, dict):
                metadata.update(maybe_metadata)
            maybe_sources = getattr(report, "sources", [])
            if isinstance(maybe_sources, list):
                sources = [item for item in maybe_sources if isinstance(item, dict)]
        else:
            text = str(report or "")
    ...
    def evaluate_report(
        ...
        Args:
            report: 生成的研究报告文本，或带 metadata 的 ResearchReport / dict。
            question_id: 对应题目的 ID。
            metrics_weights: 自定义指标权重。
            report_metadata: 可选元信息，用于接入 evidence graph / transition 指标。
        ...
        report_text, metadata, report_sources = self._extract_report_payload(report, report_metadata)
        ...
        search_policy_breakdown = SearchPolicyMetrics.breakdown(
            report_text,
            metadata,
            report_sources,
        )
        ...
        weights = dict(metrics_weights) if metrics_weights is not None else dict(base_weights)
        if metadata.get("evidence_snapshot"):
            evidence_weights = {
    ...
    def batch_evaluate(
        ...
        by_domain_metrics: dict[str, dict[str, list[float]]] = {}
        by_domain_evidence_metrics: dict[str, dict[str, list[float]]] = {}
        by_domain_search_policy_metrics: dict[str, dict[str, list[float]]] = {}
        metric_totals: dict[str, list[float]] = {}
        evidence_metric_totals: dict[str, list[float]] = {}
        search_policy_metric_totals: dict[str, list[float]] = {}

        for item in results:
            qid = item["question_id"]
            report = item.get("report", item.get("report_content", ""))
            eval_result = self.evaluate_report(
                report,
                qid,
            ...
            by_domain_metrics.setdefault(domain, {})
            by_domain_evidence_metrics.setdefault(domain, {})
            by_domain_search_policy_metrics.setdefault(domain, {})
            for key, value in eval_result.get("metrics", {}).items():
                if isinstance(value, (int, float)):
                    metric_totals.setdefault(key, []).append(float(value))
                    by_domain_metrics[domain].setdefault(key, []).append(float(value))
            for key, value in eval_result.get("evidence_metrics", {}).items():
                if isinstance(value, (int, float)):
                    evidence_metric_totals.setdefault(key, []).append(float(value))
                    by_domain_evidence_metrics[domain].setdefault(key, []).append(float(value))
            for key, value in eval_result.get("search_policy_metrics", {}).items():
                if isinstance(value, (int, float)):
                    search_policy_metric_totals.setdefault(key, []).append(float(value))
                    by_domain_search_policy_metrics[domain].setdefault(key, []).append(float(value))
        ...
        average_metrics = {
            key: sum(values) / len(values) for key, values in metric_totals.items() if values
        }
        average_evidence_metrics = {
        ...
        average_search_policy_metrics = {
            key: sum(values) / len(values)
            for key, values in search_policy_metric_totals.items()
            if values
        }
        domain_metric_avg = {
            domain: {
            ...
            for domain, metrics in by_domain_metrics.items()
        }
        domain_evidence_metric_avg = {
            domain: {
                key: sum(values) / len(values)
                for key, values in metrics.items()
                if values
            }
            for domain, metrics in by_domain_evidence_metrics.items()
        }
        domain_search_policy_metric_avg = {
            domain: {
                key: sum(values) / len(values)
                for key, values in metrics.items()
                if values
            }
            for domain, metrics in by_domain_search_policy_metrics.items()
        }

        return {
            "average_composite": avg_composite,
            "average_metrics": average_metrics,
            "average_evidence_metrics": average_evidence_metrics,
            "average_search_policy_metrics": average_search_policy_metrics,
            "by_domain": domain_avg,
            "by_domain_metrics": domain_metric_avg,
            "by_domain_evidence_metrics": domain_evidence_metric_avg,
            "by_domain_search_policy_metrics": domain_search_policy_metric_avg,
            "details": all_scores,
        }
