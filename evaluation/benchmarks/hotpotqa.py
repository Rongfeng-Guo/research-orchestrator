...
class HotpotQABenchmark:
    ...
    def get_samples(self, n: int | None = None, shuffle: bool = False, seed: int | None = None) -> list[dict[str, Any]]:
        """
        获取转换后的样本列表。
    ...
    @classmethod
    def supporting_fact_metrics(cls, pred_facts: list[Any], gold_facts: list[Any]) -> dict[str, float]:
        pred = {fact for item in pred_facts if (fact := cls.normalize_supporting_fact(item)) is not None}
        gold = {fact for item in gold_facts if (fact := cls.normalize_supporting_fact(item)) is not None}
        if not pred and not gold:
            return {"sp_em": 1.0, "sp_precision": 1.0, "sp_recall": 1.0, "sp_f1": 1.0}
        if not pred:
            return {"sp_em": 0.0, "sp_precision": 0.0, "sp_recall": 0.0, "sp_f1": 0.0}
        if not gold:
            return {"sp_em": 0.0, "sp_precision": 0.0, "sp_recall": 0.0, "sp_f1": 0.0}
        ...
        return {
            "sp_em": 1.0 if pred == gold else 0.0,
            "sp_precision": precision,
            "sp_recall": recall,
            "sp_f1": f1,
        }

    @classmethod
    def joint_metrics(cls, answer_em: float, answer_f1: float, sp_metrics: dict[str, float]) -> dict[str, float]:
        return {
            "joint_em": answer_em * sp_metrics.get("sp_em", 0.0),
            "joint_f1": answer_f1 * sp_metrics.get("sp_f1", 0.0),
        }
    ...
    def evaluate(
        ...
        for item in predictions:
            ...
            gold = item.get("gold", "")
            answer_em = 1.0 if HotpotQABenchmark.exact_match(pred, gold) else 0.0
            answer_f1 = HotpotQABenchmark.f1_score(pred, gold)
            ...
            if item.get("supporting_facts") is not None or item.get("gold_supporting_facts") is not None:
                sp = self.supporting_fact_metrics(
                    item.get("supporting_facts", []),
                    item.get("gold_supporting_facts", []),
                )
                joint = self.joint_metrics(answer_em, answer_f1, sp)
                sp_em_sum += sp["sp_em"]
                sp_f1_sum += sp["sp_f1"]
                joint_em_sum += joint["joint_em"]
                joint_f1_sum += joint["joint_f1"]
                sp_count += 1
        ...
        if sp_count:
            results["sp_em"] = sp_em_sum / sp_count
            results["sp_f1"] = sp_f1_sum / sp_count
            results["joint_em"] = joint_em_sum / sp_count
            results["joint_f1"] = joint_f1_sum / sp_count
        if entity_cov_sum > 0:
